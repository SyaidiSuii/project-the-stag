...
@section('title', 'Recommended Items - The Stag SmartDine')
...
<div class="main-content">
    ...
    <div style="background: linear-gradient(135deg, #6366f1 0%, #8b5cf6 100%); border-radius: 16px; padding: 24px; margin-bottom: 20px; box-shadow: 0 4px 16px rgba(99, 102, 241, 0.3); position: relative;">
        ...
        <div style="text-align: center; color: white;">
            <h1 style="font-size: 28px; font-weight: 700; margin: 0 0 8px 0; display: flex; align-items: center; justify-content: center; gap: 12px;">
                <i class="fas fa-robot" style="color: #fbbf24;"></i>
    ...
    <div class="search-bar-container" role="search">
        <div class="search-bar">
            ...
            <input type="text" class="search-input" placeholder="Search recommended items..." id="searchInput" aria-label="Search recommended items" />
    ...
    <!-- All Recommended Items (No Category Grouping) -->
    @if($recommendedItems->count() > 0)
        <div class="category-section" data-type="all">
            <!-- Items Grid -->
            <div class="food-grid">
                ...
                <div class="food-card" data-id="{{ $item->id }}" data-category="{{ $item->category_id }}" data-type="{{ strtolower($item->category->type ?? 'food') }}" data-name="{{ strtolower($item->name) }}">
                    <div class="food-image" style="position: relative;">
                        ...
                        <!-- Item Type Badge -->
                        <div style="position: absolute; top: 8px; left: 8px; background: linear-gradient(135deg, #6366f1 0%, #8b5cf6 100%); color: white; font-size: 10px; font-weight: 700; padding: 4px 10px; border-radius: 6px; text-transform: uppercase; letter-spacing: 0.5px; box-shadow: 0 2px 8px rgba(99, 102, 241, 0.3); z-index: 2;">
                            @php
    ...
    @else
        <div style="text-align: center; padding: 64px 32px; color: #6b7280;">
            <i class="fas fa-robot" style="font-size: 64px; color: #d1d5db; margin-bottom: 16px;"></i>
            <h3 style="font-size: 20px; color: #374151; margin: 0 0 8px 0;">No AI Recommendations Available</h3>
            <p style="font-size: 14px; margin: 0;">Start ordering to get personalized recommendations!</p>
        </div>
    @endif
...
function filterItems() {
    const allItems = document.querySelectorAll('.food-card');
    const noResults = document.getElementById('noResults');
    let totalVisibleCount = 0;

    allItems.forEach(item => {
        const itemType = (item.dataset.type || 'food').toLowerCase();
        ...
        if (currentType === 'all') {
            ...
            typeMatch = itemType.includes('food') || (!itemType.includes('drink') && !itemType.includes('beverage'));
        } else if (currentType === 'drinks') {
            // Match 'drink', 'drinks', 'beverage', etc.
            typeMatch = itemType.includes('drink') || itemType.includes('beverage');
        } else {
            typeMatch = itemType === currentType;
        }
