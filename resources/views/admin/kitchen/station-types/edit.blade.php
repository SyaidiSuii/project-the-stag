...
@section('title', 'Edit Station Type')
@section('page-title', 'Edit Station Type')
    ...
    <div class="kitchen-section">
        <div class="admin-section" style="margin-bottom: 0;">
            <div class="form-container">
                <form action="{{ route('admin.kitchen.station-types.update', $stationType->id) }}" method="POST">
                    @csrf
                    @method('PUT')

                    <div class="form-group">
                        <label for="station_type">Station Type Name <span class="text-danger">*</span></label>
                        <input type="text" id="station_type" name="station_type" value="{{ old('station_type', $stationType->station_type) }}" required placeholder="e.g., grill, bakery, sushi_bar">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="icon" name="icon" value="{{ old('icon', $stationType->icon) }}" placeholder="e.g., &#x1F525; or 🔥">
                        <small>Use HTML entity format (&#x1F525;) or direct emoji. <a href="https://emojipedia.org" target="_blank">Find emojis here</a></small>
                        @error('icon')
                            <small class="text-danger">{{ $message }}</small>
                        @enderror
                        <div class="icon-preview" id="iconPreview" style="{{ $stationType->icon ? '' : 'display: none;' }}">
                            <span id="iconDisplay">{!! old('icon', $stationType->icon) !!}</span>
                        </div>
                    </div>
