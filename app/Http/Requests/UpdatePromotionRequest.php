...
class UpdatePromotionRequest extends FormRequest
{
    ...
    {
        // Get promotion ID from route model binding
        $promotion = $this->route('promotion');
        $promotionId = $promotion instanceof Promotion ? $promotion->id : $promotion;
        ...
        switch ($promotionType) {
            case Promotion::TYPE_PROMO_CODE:
                $rules = array_merge($rules, [
                    'promo_code' => 'required|string|max:50|unique:promotions,promo_code,' . $promotionId,
            ...
            case Promotion::TYPE_SEASONAL:
                $rules = array_merge($rules, [
                    'promo_code' => 'nullable|string|max:50|unique:promotions,promo_code,' . $promotionId,
                    'discount_type' => 'required|in:percentage,fixed',
                    'discount_value' => 'required|numeric|min:0',
                    'minimum_order_value' => 'nullable|numeric|min:0',
                    'banner_image' => 'nullable|image|mimes:jpeg,png,jpg,gif|max:2048',
